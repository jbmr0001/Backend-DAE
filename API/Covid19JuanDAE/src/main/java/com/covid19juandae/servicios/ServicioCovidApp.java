...
public class ServicioCovidApp {
    ...
    public Optional<Rastreador> loginRastreador(@NotBlank String dniRastreador, @NotBlank String clave) {
        Optional<Rastreador> loginRastreador = repositorioRastreadores.buscar(dniRastreador)
                .filter((rastreador)->rastreador.claveValida(clave));
    ...
    public Optional<Usuario> verUsuario(UUID uuid) {
            Optional<Usuario> loginUsuario = repositorioUsuarios.buscar(uuid);
 
        // Asegurarnos de que se devuelve el usuario con los datos precargados
        loginUsuario.ifPresent(u -> u.getContactosCercanos().size());
        return loginUsuario;
    } 
    ...
    public Optional<Rastreador> verRastreador(@NotBlank String dni) {
        Optional<Rastreador> loginRastreador = repositorioRastreadores.buscar(dni);
 
        // Asegurarnos de que se devuelve el rastreador con los datos precargados
        loginRastreador.ifPresent(r -> r.getNumUsuariosReportados());
        return loginRastreador;
    }   
     ...
     * @return media
     */
    public double mediaContactosPositivo(String dniRastreador){
         if (!repositorioRastreadores.buscar(dniRastreador).isPresent()) {
            throw new RastreadorNoRegistrado();
         }
     ...
     */
    @Transactional
    public void notificarContactosCercano( UUID uuidUsuario, LinkedList<ContactoCercano> contactos){

        Usuario usuario = repositorioUsuarios.buscar(uuidUsuario).orElseThrow(UsuarioNoRegistrado::new);
        //Criterio de inserción para evitar repetidos
        contactos.forEach(contactoCercano -> {
            System.out.println(contactoCercano.getUsuarioContacto().getUuid());
