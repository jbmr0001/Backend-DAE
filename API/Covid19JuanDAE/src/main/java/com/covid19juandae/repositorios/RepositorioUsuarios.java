...
public class RepositorioUsuarios {
     ...
     */
    public double mediaContactosPositivos(){
        ...
        for(Usuario usuario:list){
            Query q1= em.createQuery("SELECT COUNT(u) FROM Usuario u WHERE u.uuidUsuario IN(SELECT c.usuario FROM Contacto c WHERE c.usuario=:uuid) AND u.fPositivo BETWEEN :desde AND :hasta AND u.positivo=TRUE");
        ...
        return (double) contactosPositivos/list.size();
    }
    ...
    public List<Contacto> getListaContactosCercanos(UUID uuidUsuario){
        ...
        Query q = em.createQuery("SELECT c FROM Contacto c WHERE c.usuario=:usu AND c.fecha BETWEEN :desde AND :hasta ORDER BY c.riesgo DESC");
